...
function Page() {
  ...
  useEffect(() => { (async () => {
    const { data } = await supabase.from("reponses_etudiant").select("id, statut, created_at, etapes(titre), profiles:etudiant_id(full_name, email)").eq("statut", "soumis").order("created_at", { ascending: false });
